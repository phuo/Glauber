			...
			int mode,double mu,double k)
{
   ...
   for(int i=0; i<20; i++)
     {
       sprintf(name,"hcollXY_%d",i);
       hcollXY[i] = new TH2D(name,name,200,-10,10,200,-10,10);
       hcollXY[i]->Sumw2();
       sprintf(name,"hpartXY_%d",i);
       hpartXY[i] = new TH2D(name,name,200,-10,10,200,-10,10);
       hpartXY[i]->Sumw2();
       sprintf(name,"hcollDIJ_%d",i);
       hcollDIJ[i] = new TH1D(name,name,11,-0.5,5);
       hcollDIJ[i]->Sumw2();
       sprintf(name,"himp_%d",i);
       himp[i] = new TH1D(name,name,2000,-0.005,19.995);
       himp[i]->Sumw2();
     }
   ...
   for(Int_t ievent=0;ievent<n;ievent++){
     ...
     char namea[200];
if(centbin ==4 &&  ecc_partr>0.5 )
{
TCanvas* c1 = new TCanvas("c1","c1", 800, 600);
mcg->Draw2();
sprintf(namea,"ell_%d.eps", ievent);
c1->SaveAs(namea);
}
   }
